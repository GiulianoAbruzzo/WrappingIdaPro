...
def main():
    ...
    #subprocess.check_call(['ida64.exe','-A', '-OIDAPython:script_crea_grafi.py', input])
    subprocess.check_call(['ida64.exe','-A', '-OIDAPython:script_crea_lista_funzioni.py', input])
...
def creaDizFunzioni(input):
    ...
    #apro il file che è gia salvato in formato json
    with open("ListaFunzioni_"+input+".txt","r") as f:
        diz=f.read()
    
    #elimino il .txt ora inutile
    os.remove(os.path.join(os.getcwd(), "ListaFunzioni_"+input+".txt"))
